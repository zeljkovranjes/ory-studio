...
import { updateTraitsAction, type EditTraitsState } from "./actions";
...
export function TraitsEditor({
  ...
  const initial = JSON.stringify(traits, null, 2);
  const [open, setOpen] = useState(false);
  const [value, setValue] = useState(initial);
  const [state, formAction, pending] = useActionState<EditTraitsState, FormData>(
    updateTraitsAction,
    {},
  );
  ...
  return (
    <form action={formAction}>
      ...
      <textarea
        ...
        rows={Math.min(20, Math.max(6, initial.split("\n").length + 1))}
        spellCheck={false}
      ...
      <p className="mt-1 text-xs text-fg-muted">
        Must be a JSON object that validates against the identity schema.
      </p>
